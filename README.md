...
## Real output from the captured session

Generated by actually running the pipeline and models against the captured dataset (see
`generate_images.py`) — not mockups. Gaps in the lines are real: ingestion stalled twice
during capture (see `docs/build-notes.md`), and the line is deliberately broken there rather
than drawing a fake smooth ramp across silence the system never observed.

**Mid-price and spread, real captured BTC/USDT ticks:**

![Mid-price over the captured session](images/01_mid_price.png)
![Spread in bps over time](images/02_spread_bps.png)

**Order-flow imbalance** (green = buy-side pressure, red = sell-side, bounded [-1,1]):

![Order-flow imbalance over time](images/03_order_flow_imbalance.png)

**VPIN with regime thresholds** — real Bulk Volume Classification, not a shortcut:

![VPIN over time with elevated/toxic reference lines](images/04_vpin_regimes.png)

**Alpha lens: Avellaneda-Stoikov vs naive market-making**, real walk-forward backtest:

![AS vs naive P&L and inventory](images/05_as_vs_naive_backtest.png)

**Almgren-Chriss vs TWAP** — the front-loading the model is built to produce:

![AC front-loaded trajectory vs linear TWAP](images/06_ac_vs_twap_trajectory.png)

**Risk lens: why Expected Shortfall beats VaR** at a fat-tail boundary:

![VaR sits near the safe mode, ES correctly captures the tail](images/07_var_vs_es.png)
